...
namespace Profisee.WebhookTemplate.AzureFunction.Clients.Entities
{
    public class ProfiseeEntitiesClient : BaseProfiseeClient, IProfiseeEntitiesClient
    {
        ...
        public ProfiseeEntitiesClient(HttpClient httpClient, IOptions<AppSettings> appSettings, ILogger logger)
            : base(httpClient, appSettings, logger)
        {

        }

        public async Task<GetEntityResponse> GetEntityAsync(Guid entityUId, string authorizationHeader)
        {
            const string uriFormat = "v{0}/Entities/{1}";
            var requestUri = string.Format(uriFormat, apiVersion, entityUId);

            var response = await base.GetAsync(requestUri, authorizationHeader);
